...
class ColumnSubsampler:
    ...
    def sample_indices(self) -> Tensor:
        perm = torch.randperm(self.n_features)
...
class Ensemble:
    ...
    def _build_group_perm(self, device: torch.device | None) -> Tensor:
        ...
        for g_idx in group_order.tolist():
            ...
            if size == 0:
                continue
            start = starts[g_idx]
            intra_perm = torch.randperm(size, device=device)
    ...
    def _build_members(self) -> None:
        ...
        for i in range(self._n_members):
            ...
            if self._shuffle_features:
                feature_perm = self._build_group_perm(device)

            column_indices: Tensor | None = None
            if self._column_subsampler is not None:
                column_indices = self._column_subsampler.sample_indices()
            ...
            if self._shuffle_targets and self._n_classes is not None:
                ...
                target_perm = torch.randperm(effective_n_classes, device=device)
